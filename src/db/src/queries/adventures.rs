...
pub async fn delete(repo: &Repo, _id: u64) -> Result<bool, Error> {
    let rows_affected = sqlx::query!(
        "UPDATE my_adventures SET is_deleted=0 WHERE is_deleted = 1 and id = ?",
        _id
    )
    .execute(&repo.connection_pool)
    .await?;
...
pub async fn find_latest(
    ...
    let my_adventures = sqlx::query_as!(
        MyAdventures,
        "SELECT id,title,created_at,is_deleted,image_url,item_type,link,source,journey_destiny FROM my_adventures WHERE is_deleted = 1 ORDER BY id DESC LIMIT ?, ?",
        query.offset,
        query.limit
    )
...
pub async fn find_by_item_type(
    ...
    let my_adventures = sqlx::query_as!(
        MyAdventures,
        "SELECT id,title,created_at,is_deleted,image_url,item_type,link,source,journey_destiny FROM my_adventures WHERE is_deleted = 1 AND item_type = ? ORDER BY id DESC LIMIT ?, ?",
        query.item_id,
        query.offset,
        query.limit
    )
...
pub async fn find_one(repo: &Repo, id: u64) -> anyhow::Result<Option<MyAdventures>, anyhow::Error> {
    let my = sqlx::query_as!(
        MyAdventures,
        "SELECT id,title,created_at,is_deleted,image_url,item_type,link,source,journey_destiny FROM my_adventures WHERE id = ? and is_deleted = 1",
        id
    )
