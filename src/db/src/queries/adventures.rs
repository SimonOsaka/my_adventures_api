...
pub async fn find_latest(
    ...
    let my_adventures = sqlx::query_as!(
        MyAdventures,
        "SELECT * FROM my_adventures WHERE is_deleted = 1 ORDER BY id DESC LIMIT ?, ?",
        query.offset,
        query.limit
    )
...
pub async fn find_by_item_type(
    ...
    let my_adventures = sqlx::query_as!(
        MyAdventures,
        "SELECT * FROM my_adventures WHERE is_deleted = 1 AND item_type = ? ORDER BY id DESC LIMIT ?, ?",
        query.item_id,
        query.offset,
        query.limit
    )
...
pub async fn find_one(repo: &Repo, id: u64) -> anyhow::Result<Option<MyAdventures>, anyhow::Error> {
    let my = sqlx::query_as!(
        MyAdventures,
        "SELECT * FROM my_adventures WHERE id = ? and is_deleted = 1",
        id
    )
